...
export const up = async (db: DatabaseAdapter) => {
  try {
    if (db.type === DatabaseType.postgre) {
      await db.run('ALTER TABLE invoices DROP CONSTRAINT IF EXISTS "invoices_businessId_invoiceFullNumber_key"');
      await db.run(
        `ALTER TABLE invoices
         ADD CONSTRAINT invoices_businessId_invoiceFullNumber_clientId_key
